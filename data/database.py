...
def fetch_movie_info_from_TMDB(film):
    ...
    if len(results) > 0:
        ...
        year = (
            int(response.get("release_date").split("-")[0])
        ...
        directors = ("/").join(
            [
                people.get("name")
                for people in response.get("credits", {}).get("crew", [])
        ...
        casts = ("/").join(
            [
                people.get("name")
                for people in response.get("credits", {}).get("cast", [])[:5]
            ]
        )  # pick first 5 actors
        countries = ("/").join(
            [
                country.get("name")
                for country in response.get("production_countries") or []
            ]
        )
        languages = ("/").join(
            [
                language.get("english_name")
                for language in response.get("spoken_languages", [])
            ]
        )
        genres = [genre.get("name") for genre in response.get("genres", [])]
        plot = response.get("overview") if response.get("overview") else ""
        ...
        film_details = Film_Detail(
            title=film.film_title, year=film.release_year, director=film.director
        )
    return film_details
...
def store_to_shows(show, theatre_id, cur):
    cur.execute(
        """
                ...
                RETURNING show_id
                """,
        (theatre_id, show.show_title, show.special, show.qa_with),
    )
...
def store_to_films(fd, cur):
    cur.execute(
        """
                INSERT INTO films (title, year, director, runtime, tconst, poster, backdrop, casts, countries, original_title, languages, plot)
...
def store_to_screenings(screenings, show_id, cur):
    for s in screenings:
        ...
        VALUES(%s, %s, %s, %s)
        """,
            (show_id, s.start_date, s.start_time, s.ticket_url),
        )
    return
